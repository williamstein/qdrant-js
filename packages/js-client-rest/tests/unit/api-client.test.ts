...
describe('apiClient', () => {
    ...
    test('status 200', async () => {
        ...
        await expect(apis.service.postLocks({errorMessage: 'request error', write: true})).resolves.toMatchObject({
            data: {errorMessage: 'response error'},
        });

        expect(global.fetch).toBeCalledWith(
            expect.stringMatching('http://my-domain.com'),
            expect.objectContaining({
                body: JSON.stringify({write: true, error_message: 'request error'}),
            }),
        );
    });
